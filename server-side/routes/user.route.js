...
router.get("/verify/:token", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.status(404).redirect(`${process.env.FRONTEND_URL}/notFound`);
    }

    if (user.isVerified) {
      return res.status(400).redirect(`${process.env.FRONTEND_URL}/alreadyVerified`);
    }

    if (user.verifyTokenExpiry < Date.now()) {
      return res.status(400).redirect(`${process.env.FRONTEND_URL}/tokenExpired`);
    }
